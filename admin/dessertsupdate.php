...
if (isset($_GET['id'])) {
    ...
    $stmt = $con->prepare("SELECT * FROM desserts WHERE PROID=?");
...
<body>
    <h1>Product Management - Update desserts</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
        if (empty($_FILES['image']['tmp_name'])) {
            ...
            $stmt = $con->prepare("UPDATE desserts SET PRONAME=?, PRICE=?, IMAGE=? WHERE PROID=?");
            $stmt->bind_param("ssss", $name, $price, $image, $id);
    ...
    <form method="post" action="dessertsupdate.php" enctype="multipart/form-data">
